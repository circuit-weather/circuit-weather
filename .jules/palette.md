...
**Learning:** The mobile UI layout uses a tiered vertical positioning strategy to prevent element collisions: the top-right weather widget is offset by 116px + spacing to clear the race info banner, the bottom radar controls are offset by 56px to clear map attribution, and interactive map controls (zoom, countdown) are offset by 128px from the bottom. Added `z-index: 600` to all top/bottom map overlays to ensure they sit above the dynamic race info banner and mobile header on small screens.
