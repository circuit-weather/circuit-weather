...
ors (LIVE, NEXT) to the visible label, rather than relying on external badges that might be missed.
...
## 2024-05-15 - Full Keyboard Navigation in Custom Menus
**Learning:** When implementing a custom menu (like a language selector pop-out), relying solely on Tab to navigate items is insufficient for screen reader users and power keyboard users who expect standard menu interaction patterns (Up/Down arrows, Home/End, and opening the menu via Arrow Down from the trigger).
**Action:** Always implement robust arrow key navigation for custom menus. Ensure the trigger button can open the menu using Arrow Down/Up, and implement bounded or wrapping Arrow Down/Up navigation within the menu list items, along with Home/End support and role="menuitem" for semantics.