...
**Learning:** The mobile UI layout uses a tiered vertical positioning strategy to prevent element collisions: the top-right weather widget is offset by 108px + spacing to clear the race info banner, the bottom radar controls are offset by 48px to clear map attribution, and interactive map controls (zoom, countdown) are offset by 120px from the bottom.
