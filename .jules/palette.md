...
**Learning:** The mobile UI layout uses a tiered vertical positioning strategy to prevent element collisions. The optimal offsets are programmatically calculated using Playwright to ensure an 8px vertical gap between overlapping absolute-positioned layers: 52px for the top-right weather widget to clear the race banner, 25px for the bottom radar bar to clear map attribution, and 91px for interactive map controls (zoom, countdown) to clear the radar bar.
