...
## 2026-06-05 - Console Mocking Scope and Artifact Leaks
**Learning:** Suppressing `console.warn` or `console.error` using `vi.spyOn` within a `beforeEach` block acts globally for the entire test suite, hiding genuine errors across tests that aren't explicitly expecting them. Also, outputting test commands (like coverage runs) to text files (e.g. `coverage_report.txt`) creates transient artifacts that must be explicitly cleaned up to avoid polluting the codebase.
**Action:** Always scope `console.warn` and `console.error` spies strictly within the individual test blocks (`it` blocks) that require them and immediately restore them via `.mockRestore()`. Never commit transient test output files.
