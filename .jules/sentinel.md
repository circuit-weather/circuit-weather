...
## 2026-07-02 - [Multiple-Encoding Evasion via URI Components]
**Vulnerability:** The `src/worker.js` Cloudflare Worker was vulnerable to multiple-encoding evasion techniques for path traversal and SSRF. Specifically, an attacker could double-encode a dot (`%252e`) to bypass the strict directory traversal checks (`.includes('..')`) because the path was only sliced, not fully decoded before validation.
**Learning:** Standard HTTP proxy setups or frameworks may only decode a URI component once. If an attacker encodes their payload multiple times (e.g. `%252e%252e/` for `../`), the first decode pass yields the single-encoded payload (`%2e%2e/`), which successfully evades naive string-matching validators.
**Prevention:** When validating proxied API paths (like `apiPath` or `tilePath`), ensure the paths are fully, recursively decoded (using a bounded `decodeURIComponent` loop) before applying regex validations or structural checks. Catch `URIError` during decoding to return the partially decoded string (handling legitimate '%' characters), but explicitly reject the path (e.g., returning `null`) if the bounded depth limit is reached to prevent algorithmic DoS.
