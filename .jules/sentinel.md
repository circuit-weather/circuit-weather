...
## 2024-05-18 - Nested Quantifier ReDoS in Regex Validation
**Vulnerability:** A regular expression used for CORS origin validation (`ALLOWED_PREVIEW_REGEX = /^https:\/\/(?:[a-zA-Z0-9-]+\.)*circuit-weather\.pages\.dev(?:\/|$)/`) contained a nested quantifier `(?:[a-zA-Z0-9-]+\.)*`. This allows for exponential backtracking (Regular Expression Denial of Service - ReDoS) in older JS engines if an attacker crafts a long input string (e.g., repeating `a.`) that ultimately fails to match.
**Learning:** Using nested quantifiers (like `+` inside `*`) to validate subdomains is prone to ReDoS if not bounded properly. It's often safer to flatten the repetition or use explicit, non-overlapping bounds.
**Prevention:** Replaced the nested structure with an optional group containing a flattened character class `(?:[a-zA-Z0-9.-]+\.)?` which supports multi-level subdomains but removes the exponential backtracking vector.
