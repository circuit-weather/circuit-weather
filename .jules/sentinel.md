...
## 2026-09-09 - [Insecure Rate Limiter LRU Eviction]

**Vulnerability:** The in-memory `RateLimiter` stored active IP records in a JavaScript `Map` and evicted the oldest entry using `activeStore.keys().next().value` when reaching capacity. However, in JavaScript `Map`, `set(key, value)` does not update iteration/insertion order if the key already exists in the `Map`. As a result, frequently accessed IPs were not moved to the MRU position unless deleted first, causing active, non-malicious users to be evicted prematurely while stale entries persisted.
**Learning:** In JavaScript `Map` objects, calling `map.set(key, val)` on an existing key updates its value in-place without updating its position in the iteration order. To maintain true Least Recently Used (LRU) order, `map.delete(key)` must be explicitly called before `map.set(key, val)`.
**Prevention:** Always ensure `map.delete(key)` is invoked immediately before `map.set(key, val)` whenever an entry is accessed or updated in a `Map`-backed LRU cache.
