...
const { CircuitWeatherApp } =
  await import("../public/src/CircuitWeatherApp.js");
...
const { RecentreControl } =
  await import("../public/src/map/RecentreControl.js");
const { MapWeatherWidget } =
  await import("../public/src/map/MapWeatherWidget.js");
...
describe("CircuitWeatherApp Pure Methods", () => {
  ...
  describe("renderForecastSkeleton", () => {
    beforeEach(() => {
      ...
      // Clean up any existing template mock
      const existingTemplate = document.getElementById(
        "forecast-skeleton-template",
      );
      if (existingTemplate && existingTemplate.parentNode) {
        existingTemplate.parentNode.removeChild(existingTemplate);
      }
    });

    it("renders skeleton correctly when template supports HTMLTemplateElement (.content)", () => {
      // Ensure document.getElementById returns our template mock
      const templateMock = {
        content: {
          cloneNode: vi.fn(() => {
            const frag = createMockElement("frag");
            // Use the numeric value for Node.DOCUMENT_FRAGMENT_NODE since Node is undefined in this environment
            frag.nodeType = 11;
            frag.querySelector = vi
              .fn()
              .mockReturnValue(createMockElement("box"));
            return frag;
          }),
        },
        cloneNode: vi.fn(),
      };
      vi.spyOn(document, "getElementById").mockReturnValue(templateMock);
      app.ui.forecastContent.appendChild = vi.fn();

      app.renderForecastSkeleton();

      // Verify styles and attributes
      expect(app.ui.forecastUnavailable.style.display).toBe("none");
      expect(app.ui.forecastContent.style.display).toBe("block");
      expect(app.ui.forecastContent.setAttribute).toHaveBeenCalledWith(
        "aria-busy",
        "true",
      );

      // Verify that content was cloned and appended
      expect(templateMock.content.cloneNode).toHaveBeenCalledWith(true);
      expect(app.ui.forecastContent.appendChild).toHaveBeenCalled();
    });

    it("renders skeleton correctly with fallback (template does not have .content)", () => {
      // Ensure document.getElementById returns our template mock without .content
      const firstChildMock = createMockElement("child");
      firstChildMock.className = "skeleton-fallback-box";

      const cloneMock = createMockElement("clone");

      // For the while(clone.firstChild) loop we need it to return true the first time, false the second time
      // The method is: while(clone.firstChild) { content.appendChild(clone.firstChild); }
      // In a real DOM, appendChild moves the node, so firstChild changes.
      let callCount = 0;
      Object.defineProperty(cloneMock, "firstChild", {
        get: () => {
          if (callCount === 0) {
            return firstChildMock;
          }
          if (callCount === 1) {
            return firstChildMock;
          }
      ...
      const templateMock = {
        cloneNode: vi.fn().mockReturnValue(cloneMock),
      };
      vi.spyOn(document, "getElementById").mockReturnValue(templateMock);

      app.ui.forecastContent.appendChild = vi.fn().mockImplementation(() => {
        callCount++;
      });
      ...
      // Verify that the child nodes were appended directly
      expect(templateMock.cloneNode).toHaveBeenCalledWith(true);
      expect(app.ui.forecastContent.appendChild).toHaveBeenCalledWith(
        firstChildMock,
      );
    });

    it("handles case where template is missing gracefully", () => {
      vi.spyOn(document, "getElementById").mockReturnValue(null);

      app.renderForecastSkeleton();

      // Setup attributes should still happen
      expect(app.ui.forecastUnavailable.style.display).toBe("none");
      expect(app.ui.forecastContent.style.display).toBe("block");
      expect(app.ui.forecastContent.setAttribute).toHaveBeenCalledWith(
        "aria-busy",
      ...
      // Since template is missing, nothing should be appended
  ...
  describe("getRaceEndTime", () => {
    ...
    it("returns race date + 23 hours as fallback when no time", () => {
      const race = {
        date: "2024-03-10",
        sessions: [{ id: "race", name: "Race", date: "2024-03-10" }],
      };
  ...
  describe("getGloballyNextSession", () => {
    ...
    it("returns null when all sessions are past", () => {
      app.races = [
        {
          round: "1",
          sessions: [{ id: "race", date: "2024-02-03", time: "14:00:00Z" }],
        },
      ];
  ...
  describe("selectRound", () => {
    ...
    it("updates the recentre control", () => {
      app.selectRound("1");
      expect(app.recentreControl.setCircuit).toHaveBeenCalledWith([
        26.0325, 50.5106,
      ]);
    });
    ...
    it("updates page title to race name", () => {
      ...
      expect(document.title).toBe(
        "Bahrain Grand Prix Weather - Circuit Weather",
      );
    });
  ...
  describe("selectSession", () => {
    ...
    it("updates page title with session name", async () => {
      await app.selectSession("race");
      expect(document.title).toBe(
        "Bahrain Grand Prix Race Weather - Circuit Weather",
      );
    });
  ...
  describe("renderForecast", () => {
    ...
    it("escapes windInfo.text and rotation in the forecast dashboard", () => {
      ...
      expect(app.ui.forecastContent.innerHTML).toContain(
        "&lt;script&gt;alert(&quot;xss&quot;)&lt;/script&gt;",
      );
      expect(app.ui.forecastContent.innerHTML).toContain(
        "&quot;&gt;&lt;img src=x onerror=alert(1)&gt;",
      );
      expect(app.ui.forecastContent.innerHTML).not.toContain(
        '<script>alert("xss")</script>',
      );
      expect(app.ui.forecastContent.innerHTML).not.toContain(
        '"><img src=x onerror=alert(1)>',
    ...
    it("finds the closest hourly forecast point", () => {
      ...
      const weather = {
        ...
        hourly: [
          {
            time: Math.floor(new Date("2024-03-01T13:00:00Z").getTime() / 1000),
            temp: 24,
            windSpeed: 10,
            windDir: 0,
            precipProb: 0,
          ...
          {
            time: Math.floor(new Date("2024-03-01T14:15:00Z").getTime() / 1000),
            temp: 26,
            windSpeed: 10,
            windDir: 0,
            precipProb: 0,
          ...
          {
            time: Math.floor(new Date("2024-03-01T16:00:00Z").getTime() / 1000),
            temp: 28,
            windSpeed: 10,
            windDir: 0,
            precipProb: 0,
  ...
  describe("showLoading", () => {
    it("shows loading overlay with text", () => {
      ...
      expect(app.ui.loadingOverlay.classList.toggle).toHaveBeenCalledWith(
        "visible",
        true,
      );
    });
  ...
  describe("Weather Refresh Interval Logic", () => {
    ...
    it("updateLiveWeatherForCircuit fetches forecast when circuit is selected", async () => {
      ...
      app.updateLiveWeatherForCircuit =
        CircuitWeatherApp.prototype.updateLiveWeatherForCircuit.bind(app);
      app.currentCircuitCenter = [26.0, 50.0];

      await app.updateLiveWeatherForCircuit();

      expect(app.weatherClient.getForecast).toHaveBeenCalledWith(
        26.0,
        50.0,
        expect.any(Date),
      );
  ...
  describe("bindEvents", () => {
    ...
    it("binds sessionSelect change event to selectSession", () => {
      ...
      const addEventListenerCalls =
        app.ui.sessionSelect.addEventListener.mock.calls;
      const changeEventCall = addEventListenerCalls.find(
        (call) => call[0] === "change",
      );
    ...
    it("binds roundSelect change event to selectRound when value is present", () => {
      ...
      const addEventListenerCalls =
        app.ui.roundSelect.addEventListener.mock.calls;
      const changeEventCall = addEventListenerCalls.find(
        (call) => call[0] === "change",
      );
      expect(changeEventCall).toBeTruthy();
    ...
    it("clears state when roundSelect change event has no value", () => {
      ...
      expect(app.ui.sessionSelect.innerHTML).toBe(
  ...
  describe("init Lifecycle", () => {
    ...
    it("catches initialization errors and renders error state", async () => {
      ...
      const renderErrorSpy = vi
        .spyOn(app, "renderError")
        .mockImplementation(() => {});
...
describe("Mobile Visibility", () => {
  ...
  it("bindResizeHandler calls updateMobileVisibility on query change", () => {
    ...
    expect(app.mobileQuery.addEventListener).toHaveBeenCalledWith(
      "change",
      expect.any(Function),
    );
