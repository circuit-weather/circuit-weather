...
export default {
  async fetch(request, env, ctx) {
    ...
    // Handle tile proxy requests (RainViewer tiles with caching)
...
/**
 * Handle RainViewer Tile requests with aggressive caching
 * Proxies tiles from tilecache.rainviewer.com and caches for 2 hours
 * to reduce requests to RainViewer and prevent rate limiting
 */
async function handleTileRequest(request, env, ctx) {
  const url = new URL(request.url);
  // Extract tile path: /api/tiles/v2/radar/{timestamp}/{size}/{z}/{x}/{y}/{color}/{options}.png
  const tilePath = url.pathname.replace('/api/tiles', '');

  // Validate the path format to prevent abuse
  // Expected: /v2/radar/{timestamp}/{size}/{z}/{x}/{y}/{color}/{options}.png
  const pathRegex = /^\/v2\/radar\/\d+\/\d+\/\d+\/\d+\/\d+\/\d+\/[\w_]+\.png$/;
  if (!pathRegex.test(tilePath)) {
  ...
  if (response) {
    // Cache hit - return with appropriate headers
    const headers = new Headers(response.headers);
    headers.set('X-Cache', 'HIT');
    headers.set('Access-Control-Allow-Origin', '*');
    // Client can cache for 2 hours
  ...
  try {
    ...
    const upstreamResponse = await fetch(upstreamUrl, {
      ...
      signal: AbortSignal.timeout(10000), // 10s timeout for tiles
    });

    if (!upstreamResponse.ok) {
      // Don't cache errors, just pass through
    ...
    // Prepare response for Cloudflare cache (2 hours)
    const cacheHeaders = new Headers({
      'Content-Type': 'image/png',
    ...
    // Return a transparent 1x1 PNG on error to prevent broken images
